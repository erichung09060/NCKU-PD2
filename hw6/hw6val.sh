if ! test -d ~/hw6/result ; then mkdir ~/hw6/result; fi
...
for i in {1..5}; do
	for k in {1..3}; do
		...
		result=~/hw6/result/result${i}_${k}
		...
		if (( $? != 0 )); then
			...
			echo "diff ./result/result${i}_${k} ./answer/testcase${i}/result_corpus${i}_query${i}_${k}"
		else
